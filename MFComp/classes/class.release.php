...
class Release{
	...
	public function new_release($branch){
		...
		$stmt = $this->conn->prepare("INSERT INTO tbl_release(rel_branch, rel_date_added, rel_time_added, rel_status) VALUES (?,?,?,?)");
		try {
	...
	function get_release_branch($id){
		$sql="SELECT rel_branch FROM tbl_release WHERE rel_id = :id";	
		$q = $this->conn->prepare($sql);
		$q->execute(['id' => $id]);
		$rel_branch = $q->fetchColumn();
		return $rel_branch;
	}
