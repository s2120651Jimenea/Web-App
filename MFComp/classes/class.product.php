...
public function new_product_type($tname){
	...
	$stmt = $this->conn->prepare("INSERT INTO tbl_type (type_name,type_status) VALUES (?,?)");
	try {
	...
	public function new_product($pname,$desc,$price,$type){
		...
		$stmt = $this->conn->prepare("INSERT INTO tbl_product(prod_name,prod_desc,prod_price,type_id) VALUES (?,?,?,?)");
		try {
			$this->conn->beginTransaction();
			foreach ($data as $row)
			{
	...
	public function update_product($pname,$desc, $ptype, $id){
		...
		$sql = "UPDATE tbl_product SET prod_name=:prod_name,prod_desc=:prod_desc,type_id=:type_id WHERE prod_id=:prod_id";

		$q = $this->conn->prepare($sql);
		$q->execute(array(':prod_name'=>$pname, ':prod_desc'=>$desc,':type_id'=>$ptype,':prod_id'=>$id));
		return true;
	}
	...
	function get_prod_desc($id){
		$sql="SELECT prod_desc FROM tbl_product WHERE prod_id = :id";	
		$q = $this->conn->prepare($sql);
		$q->execute(['id' => $id]);
		$prod_desc = $q->fetchColumn();
		return $prod_desc;
	}
