...
function create_new_user(){
    ...
    $result = $user->new_user($email,$password,$lastname,$firstname,$branch,$access);
    if($result){
...
function update_user(){
    ...
    $result = $user->update_user($user_id,$lastname,$firstname,$branch,$access);
    if($result){
        header('location: ../index.php?page=settings&subpage=users&action=profile&id='.$user_id);
    }
}

function list_users_search($keyword){
		
    $keyword = "%".$keyword."%";

    $q = $this->conn->prepare('SELECT * FROM `tbl_users` WHERE `user_lname` LIKE ?');
